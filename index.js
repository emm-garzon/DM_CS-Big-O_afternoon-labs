...
// ANSWER: No. A traditional for loop has a time complexity of O(n), but setting the i <= 4, makes it O(1); turning into a constant.
...
// ANSWER: No - space complexity remains the same.

const getNumbersSquared2 = (arr) => {
  let answers = [];
  for (let i = 0; i < arr.length; i++) {
    answers.push(arr[i] * arr[i]);
  }
  return answers;
};
...
// ANSWER: The string (and by extension its length) is the only dynamic variable -- all else remains constant.
